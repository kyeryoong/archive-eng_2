    ...
    <div className={styles.container}>
      {achievementsData.map((elem: AchievementsProps, index: number) => (
        <div
          className={
            hover === 0
              ? styles.achievementNormal
              : hover === index
              ? styles.achievementSelected
              : styles.achievementNotSelected
          }
        ...
        >
          <Image
            ...
            className={
              hover === index
                ? styles.achievementImageSelected
                : styles.achievementImageNotSelected
            }
          />

          <div className={styles.achievementName}>
            <div className={styles.achievementNameTop}>{elem.korTitle}</div>

            <div className={styles.achievementNameCenter}>{elem.engTitle}</div>

            <div className={styles.achievementNameBottom}>{elem.info}</div>
          </div>

          <div
            className={
              hover === index ? styles.moreButtonShow : styles.moreButtonHide
            }
            onClick={() => {
              setShowModal(true);
      ...
      >
        <div className={showModal ? styles.modalShow : styles.modalHide}>
          <div
            className={imageFull ? styles.modalTopFull : styles.modalTopNormal}
          >
            <Image
              width={1500}
              height={1500}
            ...
            <Image
              ...
              style={
                imageIndex ==
                  achievementsData[achievementIndex]?.text?.length || imageFull
                  ? { opacity: "0", cursor: "default" }
                  : {}
              }
              onClick={() => {
                if (
                  imageIndex !==
                    achievementsData[achievementIndex]?.text?.length &&
                  !imageFull
                ) {
                  setImageIndex((prev) => prev + 1);
                }
            ...
            <div
              ...
              style={imageFull ? { opacity: "0", bottom: "-20px" } : {}}
            >
              {achievementsData[achievementIndex]?.text?.map(
          ...
          <div
            className={
              imageFull ? styles.modalBottomHide : styles.modalBottomShow
            }
          >
            ...
            <div className={styles.mainText}>
              {
                achievementsData[achievementIndex]?.text[imageIndex - 1]
                  ?.mainText
              }
            </div>
